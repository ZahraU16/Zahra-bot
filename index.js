...
baby.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = ` @${num.split('@')[0]}👋 \n_Yeyyy Beban Grup Hilang 1_ , _Ucap Kan Selamat Tinggal Mwah Mwah_`
				let buff = await getBuffer(ppimg)
				baby.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
baby.sendMessage(from, lima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
				...
				if (!isGroup) return reply(ind.groupo())			
					await costum(group(pushname, prefix, botName, ownerName, getLevelingLevel, sender, _registered), text, tescuk, cr)
					break
                case 'downloadmenu':
                if (isBanned) return reply(ind.baned())
				if (!isRegistered) return reply(ind.noregis())			
...
*/
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}ninjalogo MIKU&IVAN`)
				reply(ind.wait())
		...
		case 'halloweentext':
		    ...
		    break
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub ivan&hub`)
				reply(ind.wait())
				...
				case 'nsfw':
                  ...
                  if (isBanned) return reply(ind.baned())				
					...
					if (Number(args[0]) === 1) {
						...
						reply('「SUKSES」DAH IDUP NGAB HATI2 YA PARA MEMBER SOK KERAS !')
						baby.sendMessage(from,`_WOI MEMBER LU JAN KIRIM LINK GC , NTAR GW KICK_`, text)
					} else if (Number(args[0]) === 0) {
...
case 'play':   
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Lagu Ditemukan*\nJudul : ${anu.result.title}\nSource : ${anu.result.source}\nUkuran : ${anu.result.size}\n\n_Otewe nggak pake rem_`
                buffer = await getBuffer(anu.result.thumbnail)
                lagu = await getBuffer(anu.result.url_audio)
                baby.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
                baby.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
                await limitAdd(sender)
                break
                       case 'ytmp3':
                  if (isBanned) return reply(ind.baned())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname)) 
                reply(ind.wait())
                anu = await fetchJson(`https://api.itsmeikygans.com/ytmp3?query=${body.slice(6)}&apikey=${ItsApi}`)
               if (anu.error) return reply(anu.error)
...
*/				
			...
			break
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					pok = await getBuffer(nimek)
					baby.sendMessage(from, pok, image, { quoted: mek, caption: `*PINTETEST BOS !*`})
					await limitAdd(sender)
					break 
                case 'beritahoax':
                  if (isBanned) return reply(ind.baned())
				...
				case 'info':
					...
					teks = `*❏ Nama bot* : ${me.name} 𝗥𝗮𝗺𝗹𝗮𝗻 𝗜𝗗\n*❏ AUTHOR* : ${ownerName}\n*❏ Nomor Bot* : @${me.jid.split('@')[0]}\n*❏ Prefix* : ${prefix}\n*❏ Total Block Contact* : ${blocked.length}\n*❏ Uptime* : ${kyun(uptime)}`
...
if (!isRegistered) return reply(ind.noregis())
if (isLimit(sender)) return reply(ind.limitend(pusname))
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}hartatahta NakanoMiku`)
har = body.slice(12)
reply('「❗」SABAR NGAB')
...
case 'adapokemon':
const pa = fs.readFileSync('assets/adapokemon.mp3')
...
break
case 'config':
tiga = fs.readFileSync('./assets/config.mp3');
baby.sendMessage(from, tiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4',ptt:true})
break
case 'sound4':
...
case 'slow':
					...
					media = await baby.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${media} -filter:a "atempo=1.0,asetrate=38500" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(media)
				...
				break
								case 'kicktime':
			if (isBanned) return reply(baby.only.benned)    
					if (!isGroup) return reply(baby.only.group)
					if (!isGroupAdmins) return reply(baby.only.admin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag target yang ingin di tendang!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
					...
					setTimeout( () => {
					 baby.sendMessage(from, 'SIAP SIAP NAKANO MIKU KICK NIH', text, { quoted: mek }) // ur cods
					}, 0) // 1000 = 5s,
					}
					break
										case 'spamcall':
...
break
				                case 'tomp3':
                if (isBanned) return reply(baby.only.benned)    
                return reply('❌ reply videonya kak ❌')
					reply(baby.wait)
...
break
				...
				if (!isPublic) return reply(mess.only.publikG)
					...
					case 'bass': 
					  case 'bas':
       if (isBanned) return reply(baby.only.userB)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo

					media = await baby.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${media} -af equalizer=f=45:width_type=o:width=2:g=24 ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(media)
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						baby.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
...
case 'tebalik':
				if (isBanned) return reply(baby.only.benned)    
        teb = `${body.slice(9)}`
				anu = await fetchJson(`https://videfikri.com/api/hurufterbalik/?query=${teb}`, {method: 'get'})
				teb = `${anu.result.kata}`
				reply('➸ Nih kak\n\n'+anu.result.kata)
				await limitAdd(sender) 
				break
        case 'jumlah':
				if (isBanned) return reply(baby.only.benned)  
        jum = `${body.slice(8)}`
				anu = await fetchJson(`https://videfikri.com/api/jumlahhuruf/?query=${jum}`, {method: 'get'})
				jum = `${anu.result.jumlah}`
				reply('➸ Jumlahnya'+anu.result.jumlah)
				await limitAdd(sender) 
				break
        case 'hilih':
				if (isBanned) return reply(baby.only.benned)    
        hil = `${body.slice(7)}`
				anu = await fetchJson(`https://videfikri.com/api/hilih/?query=${hil}`, {method: 'get'})
				jum = `${anu.result.kata}`
				reply('➸ Jumlahnya'+anu.result.kata)
				await limitAdd(sender) 
				break
        case 'katadilan':
				if (isBanned) return reply(baby.only.benned)    
				...
				baby.sendMessage(from, katadilan, text, {quoted: mek})
				await limitAdd(sender) 
				break  
        case 'katailham':
				if (isBanned) return reply(baby.only.benned)    
				anu = await fetchJson(`https://api-shizuka.herokuapp.com/bacotanilham?apikey=itsmeiky633`, {method: 'get'})
				katailham = `${anu.result}`
				baby.sendMessage(from, katailham, text, {quoted: mek})
				await limitAdd(sender) 
				break
					case 'info':
satu = fs.readFileSync('./database/menu/info');
baby.sendMessage(from, satu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'fast':
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
      andasiapa = `${body.slice(9)}`
			data = await fetchJson(`https://videfikri.com/api/primbon/artinama/?nama=${andasiapa}`, {method: 'get'}) 
			artiin = data.result 
			teks = `➸ Arti: ${artiin.arti}\n*➸ Desk*: ${artiin.desk}`
			baby.sendMessage(from, teks, text, {quoted: mek})
			break
							case 'kalkulator':
					if (!isRegistered) return reply(ind.noregis())
...
if (isBanned) return reply(ind.banned())
					fs.writeFileSync('./database/menu/ingfo', 
			baby.sendMessage(from, teks, text, {quoted: mek}))
break
case 'url2img':
					...
					if (!isUrl(args[1])) return reply(baby.error.Iv)
					reply(mess.wait)
				...
				case 'tsticker':
					...
					anu = await fetchJson(`https://mhankbarbar.tech/api/text2image?text=${teks}&apiKey=  "apivinz"`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(baby.error.stick)
				...
				break	
					case 'imlady':
tiga = fs.readFileSync('./assets/imlady.mp3');
baby.sendMessage(from, tiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4',ptt:true})
break
case 'lighglicht':
					var gh = body.slice(12)
if (args.length < 1) return reply(`Kirim perintah ${prefix}lighglitch ivan, contoh ${prefix}lighglicht ivan`)
					reply(baby.wait)
					anu = await fetchJson(`https://api.zeks.xyz/api/lithgtext?text=${gh}&apikey=apivinz`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					baby.sendMessage(from, buffer, image, {quoted: mek})
                                        await limitAdd(sender)
					break
					...
					anu = `https://videfikri.com/api/textmaker/glowingneon/?text=${args.join(' ')}`
					voss = await (anu)
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
						baby.sendMessage(from, await getBuffer(anu), image, { quoted: mek, caption: baby.sucess })
					} else {
	 ...
	 case 'ffbanner':
				...
				if (ween.length > 10) return reply('Teksnya kepanjangan, maksimal 9 karakter')
				reply(ind.wait())
				buffer = await getBuffer(`https://api.zeks.xyz/api/epep?text=${ween}&apikey=apivinz`)
		    baby.sendMessage(from, buffer, image, {quoted: mek})
		    await limitAdd(sender)	
		    break
		    case 'gplaybutton':
				...
				if (ween.length > 10) return reply('Teksnya kepanjangan, maksimal 9 karakter')
				reply(ind.wait())
				buffer = await getBuffer(`https://api.zeks.xyz/api/gplaybutton?text=${ween}&apikey=apivinz`)
		    baby.sendMessage(from, buffer, image, {quoted: mek})
		    await limitAdd(sender)	
		    break
case 'harrypotter':
		    ...
		    case 'woodblock':
				...
				if (ween.length > 10) return reply('Teksnya kepanjangan, maksimal 9 karakter')
				reply(ind.wait())
				buffer = await getBuffer(`https://videfikri.com/api/textmaker/woodblock/?text=${ween}`)
		    baby.sendMessage(from, buffer, image, {quoted: mek})
		    await limitAdd(sender)	
		    break
		    case 'glowingneon':
		    ...
		    break
		    				case 'afk':
                                        ...
                                        nom = mek.participant
                                        const tag = {
                                                text:  `Fitur AFK berhasil *diaktifkan!* \n\n❏ Name : @${nom.split("@s.whatsapp.net")[0]} \n❏ Alasan : ${tels}`,
